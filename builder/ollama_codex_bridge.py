...
def build_codex_prompt(
    ...
    for hit in [*atlas_hits, *shard_hits]:
        ...
        block = format_shard(hit, excerpt)
...
def format_shard(hit: RetrievedShard, excerpt: str) -> str:
    offset = f" offset={hit.offset} length={hit.length}" if hit.offset is not None else ""
    concepts = ", ".join(hit.concepts[:8])
    return "\n".join([
        f"[{hit.source}] {hit.title} score={hit.score:.2f}{offset}",
...
def run_codex(prompt: str, codex_path: str, approval: str, sandbox: str) -> int:
    ...
    try:
        completed = subprocess.run(command, cwd=ROOT, input=prompt, text=True)
